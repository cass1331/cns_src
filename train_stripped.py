...
def pair_loss(emb_t1, emb_t2, delta_npz, pred_cd, pred_hiv, labels_cd, labels_hiv, lamb, tau):
    ...
    # bce_loss_cd = focal_loss(pred_cd,labels_cd)
    bce_loss_hiv = bce(pred_hiv,labels_hiv)
    #bce_loss_hiv = focal_loss(pred_hiv,labels_hiv)
...
@torch.no_grad()
def test(feature_extractor, classifier_ucsf, loader, fold=None, epoch=None, train = False):
    ...
    for i,data in enumerate(loader):
        ...
        #     pred_1, pred_2 = torch.squeeze(preds[j]),torch.squeeze(preds[j+num_samples[i]])
            ...
            preds = torch.reshape(torch.cat((pred_1, pred_2)), (2,2))
            # print(pred_1,pred_2)
            # print(preds)
        #
        ...
        #
            ...
            # print(pred_cd1,pred_hiv1)
            labels_cd = torch.tensor([labels[0][0], labels[1][0]]).to(device).float()
            labels_hiv = torch.tensor([labels[0][1], labels[1][1]]).to(device).float()
            loss_cd = bce(pred_cd1, labels_cd).to(device)
            loss_hiv = bce(pred_hiv1, labels_hiv).to(device)
...
def train(feature_extractor,  classifier_ucsf, train_loader, test_loader,final_test_loader, fold, cf_kernel=None, lamb=0.5):
    ...
    for epoch in range(epochs):
        ...
        test_acc, test_ploss,test_accuracy_class, test_accuracy_dataset = test(feature_extractor, classifier_ucsf, test_loader, fold =fold, epoch = epoch)
        ...
        train_acc, train_ploss,train_accuracy_class, train_accuracy_dataset = test(feature_extractor, classifier_ucsf, train_loader, fold =fold, epoch = epoch, train =True)
...
#      #EXTRACT ONLY UCSF DATA
        ...
        test_acc, test_loss,test_accuracy_class, test_accuracy_dataset, test_distance = test(feature_extractor, classifier_ucsf, ucsf_test_loader, fold = fold, train =True)
