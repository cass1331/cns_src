...
from sampler import PairedSampler,PairedSamplerNPZ
...
wandb.init(project="npz disentanglement", entity="jmanasse", config = {
...
class NPZLoss(nn.Module):
    ...
    def forward(self, pred_cd1, labels_cd, pred_hiv1, labels_hiv, emb1, emb2, npz1, npz2, tau, downgraded=False):
        ...
        #compute disentanglement loss wrt to npz
        ...
        if npz1 == 99.0 or npz2 == 99.0 or math.isnan(npz1) or  math.isnan(npz2):
...
def test(feature_extractor, classifier_ucsf, loader,  ucsf_criterion, tau, fold=None, epoch=None, train = False):
    ...
    return overall_accuracy, xentropy_loss_avg, npz_loss_avg, total_loss_avg, accuracy_class, accuracy_dataset#, distance
...
def train(feature_extractor,  classifier_ucsf, train_loader, test_loader, fold ):
    ...
    for epoch in range(epochs):
        ...
        if epoch == 99:
            torch.save(feature_extractor.state_dict(), 'npz_weights.pt')
        ...
        #evaluate model
        test_acc, test_xentropy_loss,test_npz_loss,test_total_loss,test_accuracy_class, test_accuracy_dataset = test(feature_extractor, classifier_ucsf, test_loader, ucsf_criterion, feature_extractor.get_parameter('feature_extractor.tau'),fold =fold, epoch = epoch)
        ...
        train_acc, train_xentropy_loss,train_npz_loss,train_total_loss, train_accuracy_class, train_accuracy_dataset  = test(feature_extractor, classifier_ucsf, train_loader, ucsf_criterion, feature_extractor.get_parameter('feature_extractor.tau'),fold =fold, epoch = epoch, train =True)
        ...
        wandb.log({"xentropy loss": round((xentropy_loss_avg / (i + 1)),3),"npz loss": round((npz_loss_avg / (i + 1)),3),"total loss": round((total_loss_avg / (i + 1)),3),
        "test xentropy loss":round(test_xentropy_loss,3), "test npz loss":round(test_npz_loss,3), "test total loss":round(test_total_loss,3),
...
if __name__ == '__main__':
    ...
    for fold in range(0,5):
        ...
        test_loader = DataLoader(dataset=test_data ,
                                      batch_size=args.batch_size,
                                      sampler=PairedSamplerNPZ(dataset=test_data,  batch_size=args.batch_size,num_batches=batch_num_wo_replacement),# to include all test images
                                      shuffle=False,
                                      pin_memory=True,
                                      num_workers=3)
