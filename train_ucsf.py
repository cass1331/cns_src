...
wandb.init(project="baseline", entity="jmanasse", config = {
...
# ucsf_criterion_cd = nn.HingeEmbeddingLoss()
# ucsf_criterion_hiv = nn.HingeEmbeddingLoss()
...
# ucsf_criterion_hiv = torch.nn.CrossEntropyLoss(torch.tensor([(123+145)/572])).to(device)
...
def test(feature_extractor, classifier_ucsf, loader,  ucsf_criterion_cd, ucsf_criterion_hiv, fold=None, epoch=None, train = False):
    ...
    return overall_accuracy, xentropy_loss_avg, accuracy_class, accuracy_dataset, distance, roc_hiv, roc_cd
...
def train(feature_extractor,  classifier_ucsf, train_loader, test_loader,final_test_loader, cf_kernel, fold ):
    ...
    alpha2 = args.alpha[1]
    # ucsf_criterion_cd = nn.HingeEmbeddingLoss()
    ucsf_criterion_hiv = FocalLoss(alpha = alpha2, gamma = args.gamma, weights_ = torch.tensor([1/3]), dataset = 'ucsf')

    # ucsf_criterion_hiv = nn.HingeEmbeddingLoss()
    ucsf_criterion_cd = FocalLoss(alpha = alpha1, gamma = args.gamma, weights_ = torch.tensor([1/2]), dataset = 'ucsf')
    ...
    # ucsf_criterion_cd = torch.nn.CrossEntropyLoss(weight = torch.tensor([(148+145)/572, (156+123)/572])).to(device)
    # # ucsf_criterion_hiv = FocalLoss(alpha = alpha2, gamma = args.gamma, dataset = 'ucsf')
    # ucsf_criterion_hiv = torch.nn.CrossEntropyLoss(weight = torch.tensor([(123+145)/572, (156+148)/572])).to(device)
    ...
    for epoch in range(epochs):
        ...
        test_acc, test_loss,test_accuracy_class, test_accuracy_dataset, test_distance, test_roc_hiv, test_roc_cd  = test(feature_extractor, classifier_ucsf, test_loader, ucsf_criterion_cd, ucsf_criterion_hiv, fold =fold, epoch = epoch)
        ...
        train_acc, train_loss,train_accuracy_class, train_accuracy_dataset, train_distance, train_roc_hiv, train_roc_cd = test(feature_extractor, classifier_ucsf, train_loader, ucsf_criterion_cd, ucsf_criterion_hiv, fold =fold, epoch = epoch, train =True)
...
#                 ).to(device)
        ...
        test_acc, test_accuracy_class, test_accuracy_dataset, best_accuracy, best_epoch, best_models = train(feature_extractor,  classifier_ucsf, ucsf_train_loader,  ucsf_test_loader, ucsf_final_test_loader, cf_kernel=cf_kernel, fold = fold)
