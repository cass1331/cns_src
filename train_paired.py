...
def cos_loss(feature_extractor, cdr_pairs, tau):
    loss_ = 0
    # T = tau.detach().numpy()
    # for i, pair in enumerate(cdr_pairs):
    #
...
def test(feature_extractor, classifier_ucsf, loader,  ucsf_criterion_cd, ucsf_criterion_hiv, tau, fold=None, epoch=None, train = False):
    ...
    for i,(images, labels,actual_labels,datasets,ids, ages, genders) in enumerate(loader):
        ...
        classifier_ucsf[2].cfs = cfs#classifier_ucsf[2].cfs = cfs
        classifier_ucsf[5].cfs = cfs#classifier_ucsf[5].cfs = cfs
        classifier_ucsf[7].cfs = cfs#classifier_ucsf[7].cfs = cfs
    #    classifier_ucsf[8].cfs = cfs
#
        if i==0:
...
def train(feature_extractor,  classifier_ucsf, train_loader, test_loader,final_test_loader, cf_kernel, fold ):
    ...
    for epoch in range(epochs):
        ...
        tqdm.write('test_acc: %.2f u_test_acc: %.2f l_test_acc: %.2f a_test_acc: %.2f  test_xloss: %.2f test_ploss: %.2f' % (test_acc, ucsf_test_acc, lab_test_acc, adni_test_acc, test_xloss, test_ploss))
...
if __name__ == '__main__':
    ...
    for fold in range(0,5):
        ...
        classifier_ucsf = nn.Sequential(
                    ...
                    nn.ReLU(),
                     # nn.BatchNorm1d(16),
                   MetadataNorm(batch_size=args.batch_size, cf_kernel=cf_kernel, num_features = 8, trainset_size = len(train_data)),
                     nn.Linear(8, 2),
        ...
        test_acc, test_xloss,test_ploss,test_accuracy_class, test_accuracy_dataset, test_distance = test(feature_extractor, classifier_ucsf, test_loader, ucsf_criterion_cd, ucsf_criterion_hiv, best_tau,  fold = fold, train =True)
