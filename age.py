...
wandb.init(project="age disentanglement", entity="jmanasse", config = {
...
class AgeLoss(nn.Module):
    ...
    def forward(self, pred_cd1, labels_cd, pred_hiv1, labels_hiv, emb1, emb2, age1, age2, tau, downgraded=False):
        #compute BCE loss
        ...
        if age2>age1:
            ...
            #make this abs value
            age_loss = torch.sum(torch.abs(emb_len_diff-age_diff))

            return (xentropy_loss.to(device), age_loss.to(device))
        else:
            age_diff = age1 - age2
...
ucsf_criterion = AgeLoss()

@torch.no_grad()
def test(feature_extractor, classifier_ucsf, loader,  ucsf_criterion, tau, fold=None, epoch=None, train = False):
    ...
    for i,(images, labels,actual_labels,datasets,ids, ages, genders,_) in enumerate(loader):
        ucsf_pred_cd = []
        ucsf_pred_hiv = []
        ucsf_actual_labels = []
        ...
        for j in range(num_samples[i]//2):
            ...
            #get ages of pair
            age1 = ages[0][j].to(device).float()
            age2 = ages[1][j].to(device).float()
            ...
            xentropy_loss, age_loss = ucsf_criterion(pred_cd1, labels_cd, pred_hiv1, labels_hiv, feature1, feature2, age1, age2, feature_extractor.get_parameter('feature_extractor.tau'))
            
            total_loss = xentropy_loss + age_loss*args.lamb

            # normalize loss to account for batch accumulation
            xentropy_loss = xentropy_loss/accum_iter
            age_loss = age_loss/accum_iter
    ...
    return overall_accuracy, xentropy_loss_avg, age_loss_avg, total_loss_avg, accuracy_class, accuracy_dataset#, distance
...
def train(feature_extractor,  classifier_ucsf, train_loader, test_loader, fold ):
    ...
    for epoch in range(epochs):
        ...
        if epoch == 99:
            torch.save(feature_extractor.state_dict(), 'fe_weights.pt')
            torch.save(classifier_ucsf.state_dict(), 'class_weights.pt')
        ###### "Training happens here! ######

        for i, (images, labels, actual_labels, datasets, ids, ages, genders,_) in enumerate(progress_bar):
        ...
        #evaluate model
        test_acc, test_xentropy_loss,test_age_loss,test_total_loss,test_accuracy_class, test_accuracy_dataset = test(feature_extractor, classifier_ucsf, test_loader, ucsf_criterion, feature_extractor.get_parameter('feature_extractor.tau'),fold =fold, epoch = epoch)
        ...
        train_acc, train_xentropy_loss,train_age_loss,train_total_loss, train_accuracy_class, train_accuracy_dataset  = test(feature_extractor, classifier_ucsf, train_loader, ucsf_criterion, feature_extractor.get_parameter('feature_extractor.tau'),fold =fold, epoch = epoch, train =True)
        ...
        tqdm.write('test_acc: %.2f u_test_acc: %.2f test_loss: %.2f' % (test_acc, ucsf_test_acc, test_total_loss))
        ...
        wandb.log({"xentropy loss": round((xentropy_loss_avg / (i + 1)),3),"age loss": round((age_loss_avg / (i + 1)),3),"total loss": round((total_loss_avg / (i + 1)),3),
        "test xentropy loss":round(test_xentropy_loss,3), "test age loss":round(test_age_loss,3), "test total loss":round(test_total_loss,3),
