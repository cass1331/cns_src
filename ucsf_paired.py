...
def pair_maker(cdr_ids, batch_data):#, features):
    ...
    if len(has_pairs) > 0 and len(has_pairs[-1]) == 1 :
        has_pairs = has_pairs[:-1]
    if len(no_pairs) > 0 and len(no_pairs[-1]) == 1:
        no_pairs = no_pairs[:-1]
    print(len(has_cdrs), len(no_cdrs))
...
#         delta_i = (np.dot(f1-f2,T)/T_norm)
# #check line above for computing length of projection
#         id1 = ids[pair[0]]
#         id2 = ids[pair[1]]
#
#
...
def test(feature_extractor, classifier_ucsf, loader,  ucsf_criterion_cd, ucsf_criterion_hiv, tau, fold=None, epoch=None, train = False):
    ...
    for i,(images, labels,actual_labels,datasets,ids, ages, genders) in enumerate(loader):
        ...
        paired_loss = None
        if len(cdr_pairs) == 0:
            paired_loss = torch.tensor([0.]).to(device)
        else:
            paired_loss = pair_loss([cdr_pairs[0]], feature, ids, tau,  len([cdr_pairs[0]])).to(device) #+ #cos_loss(feature_extractor, cdr_pairs, tau)

        xentropy_loss = xentropy_loss1 + xentropy_loss2
        xentropy_loss = xentropy_loss * 0
        total_loss = xentropy_loss + paired_loss
    ...
    return overall_accuracy, xentropy_loss_avg,paired_loss_avg, accuracy_class, accuracy_dataset, distance
...
def train(feature_extractor,  classifier_ucsf, train_loader, test_loader,final_test_loader, cf_kernel, fold ):
    ...
    for epoch in range(epochs):
        ...
        #    print(cdr_pairs)
            ...
            xentropy_loss = xentropy_loss * 0
            total_loss = xentropy_loss + paired_loss
            #check if paired loss alone can converge correctly
        ...
        test_acc, test_xloss, test_ploss, test_accuracy_class, test_accuracy_dataset, test_distance = test(feature_extractor, classifier_ucsf, test_loader, ucsf_criterion_cd, ucsf_criterion_hiv, tau, fold =fold, epoch = epoch)
        ...
        train_acc, train_xloss, train_ploss,train_accuracy_class, train_accuracy_dataset, train_distance = test(feature_extractor, classifier_ucsf, train_loader, ucsf_criterion_cd, ucsf_criterion_hiv, tau, fold =fold, epoch = epoch, train =True)
        ...
        tqdm.write('test_acc: %.2f u_test_acc: %.2f test_xloss: %.2f test_ploss: %.2f' % (test_acc, ucsf_test_acc, test_xloss, test_ploss))
        row = {'epoch': epoch, 'train_acc': round(overall_accuracy,3), 'test_acc': test_acc, 'train_xloss':round((xentropy_loss_avg / (i + 1)),3), 'train_ploss':round((paired_loss_avg / (i + 1)),3), 'test_xloss': round(test_xloss,3), 'test_ploss': round(test_ploss,3),
...
if __name__ == '__main__':
    ...
    csv_logger = CSVLogger( args, fieldnames=['epoch', 'train_acc',  'test_acc',
                                              'train_xloss','train_ploss','test_xloss','test_ploss',
...
#                 ).to(device)
        ...
        ucsf_train_loader = DataLoader(dataset=ucsf_data,
                                  batch_size=args.batch_size,
                                  # sampler=ClassMixedSampler(dataset=ucsf_data,
                                  #                      batch_size=args.batch_size),
                                  shuffle=True,
                                  pin_memory=True,
                                  num_workers=3)
        ucsf_test_loader = DataLoader(dataset=ucsf_test_data,
                                  batch_size=args.batch_size,# to include all test images
                                  shuffle=True,
                                  pin_memory=True,
                                  num_workers=3)
        ucsf_final_test_loader = DataLoader(dataset=ucsf_test_data ,
                          batch_size=1,#args.batch_size,
                          shuffle=False,
                          pin_memory=True,
                          num_workers=3)
        ...
        feature_extractor = fe(cf_kernel, trainset_size = len(ucsf_data), in_num_ch=1, img_size=(64, 64, 64), inter_num_ch=16,
                           fc_num_ch=16, kernel_size=3, conv_act='relu',
                           fe_arch=args.fe_arch, dropout=args.dropout,
                           fc_dropout = args.fc_dropout, batch_size = args.batch_size).to(device)
        ...
        classifier_ucsf = nn.Sequential(
                    ...
                    nn.ReLU(),
                     # nn.BatchNorm1d(16),
                   MetadataNorm(batch_size=args.batch_size, cf_kernel=cf_kernel, num_features = 8, trainset_size = len(ucsf_data)),
                     nn.Linear(8, 2),
                     MetadataNorm(batch_size=args.batch_size, cf_kernel=cf_kernel, num_features = 2, trainset_size = len(ucsf_data)),
                ).to(device)

        test_acc, test_accuracy_class, test_accuracy_dataset, best_accuracy, best_epoch, best_tau, best_models = train(feature_extractor,  classifier_ucsf, ucsf_train_loader,  ucsf_test_loader, ucsf_final_test_loader, cf_kernel=cf_kernel, fold = fold)
        ...
        test_acc, test_loss,test_accuracy_class, test_accuracy_dataset, test_distance = test(feature_extractor, classifier_ucsf, ucsf_test_loader, ucsf_criterion_cd, ucsf_criterion_hiv,  best_tau,fold = fold, train =True)
