...
from sampler import SuperSampler,UCSFSampler
...
# alpha1 = args.alpha[0]
# alpha2 = args.alpha[1]
ucsf_criterion_cd = FocalLoss()
ucsf_criterion_hiv = FocalLoss()
...
def test(feature_extractor, classifier_ucsf, loader,  ucsf_criterion_cd, ucsf_criterion_hiv, fold=None, epoch=None, train = False):
    ...
    for i,(images, labels,actual_labels,datasets,ids, ages, genders, npzs) in enumerate(loader):
        ...
        if i == num_batches - 1:
            ...
            images0, labels0, actual_labels0, datasets0, ids0, ages0, genders0, npzs0 = first_batch_data
            ...
            npzs = np.concatenate((npzs, npzs0[:number_needed]),axis=0)

        data = (images, labels, actual_labels, datasets, ids, ages, genders, npzs)
...
def get_cf_kernel(loader):
    ...
    for i,(all_images, all_labels, all_actual_labels, all_datasets, all_ids, all_ages, all_genders, all_npzs) in enumerate(loader):
...
def get_cf_kernel_batch(data):
    all_images, all_labels, all_actual_labels, all_datasets, all_ids, all_ages, all_genders, all_npzs = data
    label_hiv = []
    label_cd = []
    ages = []
    gender_m = []
...
def train(feature_extractor,  classifier_ucsf, train_loader, test_loader,final_test_loader, cf_kernel, fold ):
    ...
    for epoch in range(epochs):
        ...
        if epoch == 99:
            torch.save(feature_extractor.state_dict(), 'fe_mdn_weights.pt')
            torch.save(classifier_ucsf.state_dict(), 'class_mdn_weights.pt')
        ...
        #        'ucsf_train_acc': ucsf_train_acc, 'ucsf_test_acc': ucsf_test_acc, 
        ...
        wandb.log({"xentropy loss": round((xentropy_loss_avg / (i + 1)),3), "test xentropy loss":round(test_loss,3),
...
if __name__ == '__main__':
    ...
    for fold in range(0,5):
        ...
        train_loader = DataLoader(dataset=train_data,
                                  batch_size=args.batch_size,
                                  sampler= UCSFSampler(dataset=train_data,
                                                       batch_size=args.batch_size),
                                  shuffle=False,
                                  pin_memory=True,
                                  num_workers=3)
